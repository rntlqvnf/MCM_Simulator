...
void RangeSlider::setLowerValue(int aLowerValue)
{
    if(mUpperValue - aLowerValue > SLIDER_TIME_RATE * GRAPH_X_LIMITATION){
        int diff = mLowerValue - aLowerValue;
        if(!(aLowerValue < mMinimum)) mUpperValue -= diff;
    }
...
void RangeSlider::setUpperValue(int aUpperValue)
{
    if(aUpperValue - mLowerValue > SLIDER_TIME_RATE * GRAPH_X_LIMITATION) {
        int diff = aUpperValue - mUpperValue;
        if(!(aUpperValue > mMaximum)) mLowerValue += diff;
    }
